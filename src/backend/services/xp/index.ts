...
export function calculateRunXP(
    ...
    const baseXP = runs.reduce((sum, run) => {
        return sum + Math.round(run.distance * XP_PER_KM) + Math.round(run.duration * XP_PER_MIN);
    }, 0);
...
export async function syncUserXP(userId: string, runs: RunData[]) {
    ...
    const user = await db.user.findUnique({
        where: { id: userId },
        select: { xp: true, currentStreak: true, longestStreak: true, lastRunDate: true },
    });

    if (!user) return;

    const latestRunDate = runs.reduce((latest, r) =>
        r.date > latest ? r.date : latest, runs[0]!.date
    );
